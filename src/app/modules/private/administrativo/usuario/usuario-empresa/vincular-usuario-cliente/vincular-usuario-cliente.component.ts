...
import {
  ClienteAsignacion
} from '../../../../../../apis/model/module/private/administrativo/usuario/response/cliente-asignacion-response';
...
import {UsuarioClienteService} from '../../../../../../service/modules/private/administrativo/usuario-cliente.service';
...
@Component({
  selector: 'app-vincular-usuario-cliente',
  imports: [
    Button,
    Dialog,
    ReactiveFormsModule,
  ...
  templateUrl: './vincular-usuario-cliente.component.html',
  styleUrl: './vincular-usuario-cliente.component.scss'
})
export class VincularUsuarioClienteComponent implements OnChanges {
  ...
  constructor(
    private readonly usuarioClienteService: UsuarioClienteService,
  ...
  get headerTitle(): string {
    return `Gestionar Empresas - ${this.usuarioNombre}`;
  }
  ...
  private cargarClientes() {
    ...
    this.isLoading = true;
    this.usuarioClienteService.obtenerClientesParaTransferencia(this.usuarioId).subscribe({
      next: (response: UsuarioClientesTransfer) => {
      ...
      error: (error) => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'No se pudieron cargar los clientes del usuario'
        });
        this.isLoading = false;
        this.cerrar();
      }
  ...
  private transferirClientes(clientesIdsAsignar: number[], clientesIdsDesasignar: number[]) {
    ...
    const request: TransferirClientesRequest = {
      usuarioId: this.usuarioId,
      clientesIdsAsignar: clientesIdsAsignar,
      clientesIdsDesasignar: clientesIdsDesasignar
    };

    this.usuarioClienteService.transferirClientes(request).subscribe({
      next: () => {
        this.messageService.add({
          severity: 'success',
          summary: 'Éxito',
